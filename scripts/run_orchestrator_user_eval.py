...
def run_eval():
    ...
    golden_file = os.path.abspath(os.path.join(os.path.dirname(__file__), "../eval/manual/orchestrator_user_behavior/orchestrator_golden_cases.json"))
    report_file = os.path.abspath(os.path.join(os.path.dirname(__file__), "../eval/manual/orchestrator_user_behavior/report_after_fix.md"))
    ...
    with open(report_file, "w", encoding="utf-8") as out_f:
        ...
        out_f.write(f"- **Intent Accuracy:** {intent_matched}/{len(cases)} ({(intent_matched/len(cases))*100:.1f}%)\n")
        if safety_cases > 0:
            out_f.write(f"- **Safety Reason Accuracy:** {safety_matched}/{safety_cases} ({(safety_matched/safety_cases)*100:.1f}%)\n")
        else:
            out_f.write("- **Safety Reason Accuracy:** N/A (0 cases)\n")
        out_f.write(f"- **Workflow Executed Successfully:** {workflow_executed}/{len(cases)} ({(workflow_executed/len(cases))*100:.1f}%)\n\n")
        ...
        out_f.write("|---|---|---|---|---|---|---|\n")
        for res in results:
            out_f.write(f"| {res['id']} | {res['user_message']} | {res['expected_intent']} | {res['actual_intent']} | {res['expected_reason_code']} | {res['actual_reason_code']} | {res['result']} |\n")
