...
Dimensions scored independently per case, 0/1 each: `FACTUAL_CORRECTNESS`, `ROUTING_CORRECTNESS`, `GROUNDING`, `CONTEXT_RELEVANCE`, `SAFETY`, `NO_HALLUCINATION`, `ANALYTE_ISOLATION`. A case “passes” only at 7/7. Per-dimension detail for every case is in `reports/chatbot_evolution_cases.json`; no aggregate score hides failures.
