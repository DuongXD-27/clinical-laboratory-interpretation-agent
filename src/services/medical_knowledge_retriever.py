...
# Note types that are relevant for a given classified status. The description
# is always included as neutral background context alongside the status note.
_STATUS_NOTE_TYPES: dict[str, tuple[str, ...]] = {
...
# The single most-specific note type for a status, used to rank/boost chunks.
_STATUS_PRIMARY_NOTE: dict[str, str] = {
    "high": "high_note",
    "low": "low_note",
    "critical_high": "critical_high_note",
    "critical_low": "critical_low_note",
    "normal": "description",
    "unknown": "description",
}
...
class ChromaMedicalKnowledgeRetriever:
    ...
    def _status_query(self, query: str, status: str) -> str:
    ...
    @staticmethod
    def _note_types_for_status(status: str) -> tuple[str, ...]:
        return _STATUS_NOTE_TYPES.get(status, _STATUS_NOTE_TYPES["unknown"])
    ...
    def _metadata_prong(
        ...
        *,
        ...
        note_types = self._note_types_for_status(status)
        try:
            result = self.vector_store.get_by_metadata(
                filter={
                    "$and": [
                        {"analyte_id": analyte_id},
                        {"note_type": {"$in": list(note_types)}},
                    ]
                },
    ...
    def retrieve(
        ...
        *,
        ...
        status_query = self._status_query(query, status)
        ...
        dense_k = max(limit * 2, len(self._note_types_for_status(status)))
        candidates.extend(
            self._dense_prong(
