...
@pytest.mark.asyncio
async def test_doctor_query_for_doctor_username_returns_404(client):
    doctor = await login_headers(client, "bacsi", "bacsi123")

    listing = await client.get("/api/v1/history", headers=doctor, params={"patient_username": "bacsi"})
